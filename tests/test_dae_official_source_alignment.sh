...
grep -q '^PKG_SOURCE_URL:=https://github.com/olicesx/dae.git$' "$DAE_MAKEFILE" || {
	echo "dae Makefile is not aligned to the kdae upstream"
	exit 1
}

grep -q '^GIT_BRANCH:=kdae$' "$DAE_MAKEFILE" || {
	echo "dae Makefile is not pinned to the kdae branch"
	exit 1
}

grep -q 'https://github.com/olicesx/outbound.git' "$DAE_MAKEFILE" || {
	echo "dae Makefile does not use the kdae outbound replacement"
	exit 1
}

grep -q 'https://github.com/olicesx/quic-go.git' "$DAE_MAKEFILE" || {
